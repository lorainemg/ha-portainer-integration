...
TEST(ApplyTest, ApprovedNewStackAutoPopulatesContainers) {
    YamlState state;
    DiffReport diff;
    diff.new_stacks.push_back({14, "monitoring", "monitoring",
                               {"grafana", "prometheus"}});
    ...
    ASSERT_EQ(state.stacks.size(), 1);
    ASSERT_EQ(state.stacks[0].containers.size(), 2);
    EXPECT_EQ(state.stacks[0].containers[0].slug, "grafana");
    EXPECT_EQ(state.stacks[0].containers[0].name, "grafana");
    EXPECT_EQ(state.stacks[0].containers[0].icon, "mdi:application-outline");
    EXPECT_EQ(state.stacks[0].containers[1].slug, "prometheus");
}

TEST(ApplyTest, DeclinedNewStackWithContainersStillGoesToIgnored) {
    YamlState state;
    DiffReport diff;
